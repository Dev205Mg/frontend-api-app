          ...
          <div class="flex items-center">
            ...
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                <RouterLink v-for="nav in navigation" :key="nav.name" :to="nav.to"
                  :class="[
                    $route.name === nav.to.name ? 'bg-gray-900 text-white'
                      : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']"
                  :aria-current="$route.name === nav.to.name ? 'page' : undefined">
                  {{ nav.name }}
                </RouterLink>
              </div>
            </div>
      ...
      <DisclosurePanel v-if="user" class="md:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
          <RouterLink v-for="nav in navigation" :key="nav.name" :to="nav.to"
            ...
            :aria-current="$route.name === nav.to.name ? 'page' : undefined">{{ nav.name }}</RouterLink>
        </div>
        <div class="border-t border-gray-700 pt-4 pb-3">
          <div class="flex items-center px-5">
            <div class="shrink-0">
              <img class="size-10 rounded-full" :src="userAuth.imageUrl" alt="" />
            </div>
            <div class="ml-3">
              <div class="text-base/5 font-medium text-white">{{ userAuth.name }}</div>
              <div class="text-sm font-medium text-gray-400">{{ userAuth.email }}</div>
            </div>
          </div>
